...
const props = withDefaults(defineProps<PrestamoFormProps>(), {
  prestamoId: null,
})

const emit = defineEmits<{
  saved: [payload?: PrestamoFormSavedPayload]
  created: [payload: PrestamoFormSavedPayload]
  cancel: []
}>()
...
const isLoadingPrestamo = computed(
  () => !isCreateMode.value && props.active && prestamoQuery.isFetching.value,
)
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    if (isCreateMode.value) {
      ...
      const payload: PrestamoFormSavedPayload = {
        id: created.id,
        idCliente: created.id_cliente ?? fields.idCliente,
      }
...
watch(
  () => [props.active, props.mode, props.prestamoId] as const,
  ([isActive, mode, prestamoId]) => {
    if (!isActive) return
    ...
    if (
      mode === 'edit' &&
      prestamoId &&
      activePrestamoId.value === prestamoId &&
      internalMode.value === 'edit'
    ) {
      detalleFilters.value = {
...
watch(
  ...
  () => {
    if (props.active && activePrestamoId.value) {
      syncFormValues()
    }
  },
)
