<template>
  <div class="space-y-6">
    <PageBreadcrumb page-title="Retenciones" :items="breadcrumb" />

    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Retenciones</h1>
      <button
        type="button"
        ...
        @click="router.push({ name: 'admin-retenciones-nueva' })"
      >
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
          <thead>
            <tr>
              ...
              <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400">Retenido</th>
              <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500 dark:text-gray-400">Pagado</th>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            ...
            <tr v-else-if="registros.length === 0">
              <td colspan="9" class="px-4 py-8 text-center text-sm text-gray-500">No hay retenciones registradas</td>
            </tr>
            <tr
              v-for="item in registros"
              :key="item.id"
              class="cursor-pointer transition hover:bg-gray-50 dark:hover:bg-white/[0.04]"
              @click="router.push({ name: 'admin-retenciones-detalle', params: { id: item.id } })"
            >
              ...
              <td class="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{{ formatMoney(item.monto_retenido) }}</td>
              <td class="px-4 py-3 text-sm text-right text-gray-700 dark:text-gray-300">{{ formatMoney(item.monto_pagado) }}</td>
              ...
              <td class="px-4 py-3 text-center">
                <button
                  ...
                  @click.stop="router.push({ name: 'admin-retenciones-detalle', params: { id: item.id } })"
                >
                  Ver
                </button>
              </td>
...
import type { RetencionListItem } from '../interfaces/retencion.interface'
...
const registros = ref<RetencionListItem[]>([])
...
async function cargar() {
  cargando.value = true
  try {
    const resp = await retencionesService.listar({
