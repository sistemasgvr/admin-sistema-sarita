      ...
      <div v-if="cliente" class="space-y-4">
        ...
        >
          ...
          <div v-if="activeTab === 'solicitudes'">
            <RelatedListState
              :loading="solicitudesQuery.isLoading.value"
              :error="solicitudesQuery.isError.value"
              :empty="!solicitudesItems.length"
              empty-text="Este cliente no tiene solicitudes registradas."
              @retry="solicitudesQuery.refetch()"
            >
              <div class="overflow-x-auto">
                <table class="w-full text-sm">
                  <thead>
                    <tr
                      class="border-b border-gray-200 text-left text-xs font-medium text-gray-500 dark:border-gray-700"
                    >
                  ...
                  <tbody>
                    <tr
                      v-for="item in solicitudesItems"
                      :key="item.id"
                    ...
                    >
                      ...
                      <td class="py-2 pr-3">
                        <AppBadge
                          size="sm"
                          :color="
                            item.nombre_estado_aprobacion === 'APROBADA'
                              ? 'success'
                              : item.nombre_estado_aprobacion === 'RECHAZADA'
                                ? 'error'
                                : 'warning'
                          "
                        >
  ...
  type RelatedTabKey = 'contactos' | 'direcciones' | 'choferes' | 'vehiculos' | 'cuentas-bancarias' | 'solicitudes'
...
// Solicitudes (bajas / reactivaciones)
...
const solicitudesItems = computed<BajaCliente[]>(() => solicitudesQuery.data.value?.data ?? [])
  ...
  },
  {
    ...
    icon: ICONS.fileText,
    count:
      solicitudesQuery.data.value?.meta.total ??
      (solicitudesQuery.isSuccess.value ? solicitudesItems.value.length : null),
  },
])
