      ...
      <div class="grid gap-3 sm:grid-cols-3">
        ...
        <div class="sm:col-span-2">
          <div class="flex items-start gap-2">
            ...
            <div class="shrink-0">
              <span class="invisible mb-1.5 block text-sm font-medium">&nbsp;</span>
              <button
                type="button"
...
import { AppCheckbox, AppInput, AppSelect, AppTextarea, MapaLeaflet } from '@/shared/components'
...
const idParaEditar = computed(() => (props.mode === 'edit' ? props.clienteId : undefined))
...
const handleConsultarDocumento = async () => {
  ...
  try {
    // Verificamos duplicado ANTES de llamar a RENIEC/SUNAT y de autocompletar.
    // Antes ambos flujos corrían por separado (este botón vs. @focusout del campo),
    // lo que producía un toast de éxito ("Datos de RENIEC cargados") a la vez que
    // aparecía el error "ya está registrado" bajo el campo: contradictorio y confuso
    // sobre si el registro se puede crear. Si ya está registrado, no tiene sentido
    // autocompletar un formulario de creación con esos datos.
    documentoDuplicado.value = await documentoYaRegistrado(
      numero,
      props.mode === 'edit' ? props.clienteId : undefined,
    )
    if (documentoDuplicado.value) {
      setFieldError('numeroDocumento', 'Este número de documento ya está registrado')
      toastWarning('Este documento ya está registrado. No se autocompletarán datos.')
      return
    }
...
watch(
  ...
  (data) => {
    if (props.active && props.mode === 'edit' && data) {
      void syncFormValues()
    }
  },
  // `immediate` es indispensable aquí: si el detalle ya está en caché de vue-query
  // (p.ej. el usuario visitó "Ver ficha" hace <30s, ver staleTime en useClienteDetailQuery),
  // detailQuery.data.value llega con datos desde el primer render y este watcher nunca
  // detecta un "cambio" porque no hay transición de undefined -> data. Sin immediate,
  // el formulario se queda con los initialValues vacíos aunque el cliente sí exista
  // (bug reportado: formulario de edición en blanco pese a que las pestañas relacionadas
  // sí cargan, porque esas usan sus propias queries).
  { immediate: true },
)
</script>
