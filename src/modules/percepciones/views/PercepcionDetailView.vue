<template>
  <div class="space-y-6">
    <PageBreadcrumb page-title="Percepción" :items="breadcrumb" />

    <div v-if="cargando" class="text-center py-8 text-gray-500">Cargando...</div>

    <template v-else-if="percepcion">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">
            Percepción {{ percepcion.serie }}-{{ percepcion.numero }}
          </h1>
          <p class="text-sm text-gray-500">Emitida el {{ formatFecha(percepcion.fecha_emision) }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
          ...
          -->
          <button
            v-if="percepcion.nombre_estado_sunat === 'ACEPTADO'"
            type="button"
          ...
          >
            ...
            {{ emitiendo ? 'Emitiendo...' : percepcion.nombre_estado_sunat === 'RECHAZADO' ? 'Reintentar emisión' : 'Emitir a SUNAT' }}
          </button>
        </div>
      </div>
      ...
      <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.02]">
        <h2 class="mb-4 text-sm font-semibold text-gray-900 dark:text-white">Datos de la percepción</h2>
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <div>
            <span class="text-xs text-gray-500">Serie</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ percepcion.serie }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Número</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ percepcion.numero }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Régimen</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ percepcion.regimen }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Tasa</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ percepcion.tasa }}%</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Base imponible</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatMoney(percepcion.base_imponible) }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Monto percibido</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatMoney(percepcion.monto_percibido) }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Monto cobrado</span>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ formatMoney(percepcion.monto_cobrado) }}</p>
          </div>
          <div>
            <span class="text-xs text-gray-500">Estado SUNAT</span>
            <AppBadge v-if="percepcion.nombre_estado_sunat" size="sm" :color="percepcion.nombre_estado_sunat === 'ACEPTADO' ? 'success' : percepcion.nombre_estado_sunat === 'RECHAZADO' ? 'error' : 'warning'">
              {{ percepcion.nombre_estado_sunat }}
            </AppBadge>
            <span v-else class="text-xs text-gray-400">Sin emitir</span>
          </div>
        </div>
        <div v-if="percepcion.observacion" class="mt-3">
          <span class="text-xs text-gray-500">Observaciones</span>
          <p class="text-sm text-gray-700 dark:text-gray-300">{{ percepcion.observacion }}</p>
        </div>
      </div>

      <!-- Detalle -->
      <div v-if="percepcion.detalles?.length" class="rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.02]">
          ...
          <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
            <thead>
              <tr>
                ...
                <th class="px-4 py-3 text-right text-xs font-semibold text-gray-500">Percibido</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
              <tr v-for="d in percepcion.detalles" :key="d.id">
                ...
                <td class="px-4 py-3 text-sm text-right text-gray-700">{{ formatMoney(d.imp_percibido) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
import type { Percepcion } from '../interfaces/percepcion.interface'
...
const percepcion = ref<Percepcion | null>(null)
...
async function cargar() {
  cargando.value = true
  try {
    const resp = await percepcionesService.obtenerPorId(Number(route.params.id))
    percepcion.value = resp.registro
  } catch {
    percepcion.value = null
  } finally {
...
async function onEmitir() {
  if (!percepcion.value) return
  emitiendo.value = true
  try {
    await percepcionesService.emitir(percepcion.value.id)
    toastSuccess('Percepción emitida a SUNAT')
    await cargar()
  } catch (error) {
    toastApiError(error)
  } finally {
...
async function onVerPdfOficial() {
  ...
  try {
    const blob = await percepcionesService.obtenerPdfOficial(percepcion.value.id)
...
async function onDescargarXmlOficial() {
  ...
  descargandoPdfXml.value = true
  try {
    const blob = await percepcionesService.obtenerXmlOficial(percepcion.value.id)
    const a = document.createElement('a')
    a.href = URL.createObjectURL(blob)
    a.download = `PERCEPCION-${percepcion.value.serie}-${percepcion.value.numero}.xml`
    a.click()
